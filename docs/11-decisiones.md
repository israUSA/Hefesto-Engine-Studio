...
### ADR-012 · Claves en la app, cifradas con DPAPI (no en `.env`)
**Contexto:** es una app de escritorio: el usuario no edita archivos, y `.env` guarda las claves en texto plano.
**Decisión:** las claves se cargan en Ajustes → Proveedores (mientras no hay UI, con `nx run api:cli keys set`) y se guardan cifradas con **Windows DPAPI** (ámbito del usuario) en `HEFESTO_HOME/secrets.json`. La `SecretVault` las descifra una vez al arrancar y las mantiene en memoria. La base guarda solo el nombre (`secretRef`) y la UI recibe solo los últimos 4 caracteres. `.env` queda como respaldo para desarrollo. Reemplaza la regla "secretos solo en `.env`".
**Consecuencias:** sin dependencias nativas (DPAPI vía PowerShell). La bóveda solo se abre con la misma cuenta de Windows: si se copia `HEFESTO_HOME` a otra PC, hay que volver a cargar las claves. Los tokens OAuth de Drive (Fase 3) usan la misma bóveda.
